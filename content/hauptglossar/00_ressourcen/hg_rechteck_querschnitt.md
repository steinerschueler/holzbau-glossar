...
quellenkonflikt: |
  ...
  Konvention zur Definition der lokalen 2D-Achsen (û, v̂) in der
  Querschnittsebene:

  - **û** = lokale **y-Achse** des Bauteils (Breitenrichtung,
    parallel zur Breitenkante b).
  - **v̂** = lokale **z-Achse** des Bauteils (Höhenrichtung,
    parallel zur Höhenkante h).
  - Das lokale 3D-System des Bauteils ist (x̂_lokal, ŷ_lokal, ẑ_lokal)
    mit x̂_lokal entlang der Bauteilachse, ŷ_lokal = û in der
    Querschnittsebene und ẑ_lokal = v̂ in der Querschnittsebene
    (rechtshändig, x̂ × ŷ = ẑ).
...
- E ⊂ ℝ³ die Querschnittsebene (siehe `ebene`) mit lokalem
  rechtshändigem Orthonormalsystem (û, v̂) ⊂ E im Flächenschwerpunkt
  z ∈ E als Ursprung,
...
```
Q(RQ) := { z + u·û + v·v̂ ∈ ℝ³ | (u, v) ∈ Q_lokal(b, h) }.
```
...
```
v_i^W = z + u_i · û + v_i · v̂   für i = 1, …, 4.
```
...
Damit gilt z = z + 0·û + 0·v̂, d. h. der Flächenschwerpunkt in W
...
- **Existenz**: Für b > 0 und h > 0 ist Q_lokal(b, h) eine
  ...
  in ℝ³ über die affine Abbildung (u, v) ↦ z + u·û + v·v̂ ist eine
  Isometrie und überträgt diese Eigenschaften.
- **Eindeutigkeit der Punktmenge**: Q_lokal(b, h) ist durch (b, h)
  eindeutig festgelegt; Q(RQ) ist durch (E, z, û, v̂, b, h) eindeutig
  festgelegt.
...
- **Wohldefiniertheit des lokalen Systems (û, v̂)**: Die Wahl von
  (û, v̂) als rechtshändiges Orthonormalsystem in E bestimmt die
  ...
  **Festlegung des Bauteils**, nicht des Querschnitts (siehe
  ...
  Querschnitts wird (û, v̂) als gegeben vorausgesetzt. Die Punktmenge
...
- **IFC**: `IfcRectangleProfileDef(XDim = b, YDim = h, Position =
  IfcAxis2Placement2D(Location = Schwerpunkt, RefDirection = û))`;
...
- **Lokales 2D-System (Konvention)**: û = Bauteil-y (Breitenrichtung,
  parallel zu b), v̂ = Bauteil-z (Höhenrichtung, parallel zu h). Diese
...
- **IFC-Mapping**:
  ...
  - `IfcRectangleProfileDef.Position` ← `IfcAxis2Placement2D` mit
    Location im Schwerpunkt (lokal Origin) und RefDirection als û.
